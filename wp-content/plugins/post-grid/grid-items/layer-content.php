...
if ( ! defined('ABSPATH')) exit;  // if direct access
	...
	foreach($layout as $item_id=>$item_info){
		...
		elseif($item_key=='excerpt_read_more'){
			...
			$lang = get_locale();
			if ($lang == 'vi') {
				$read_more_text = apply_filters('post_grid_filter_grid_item_read_more', __('.. Đọc tiếp »', post_grid_textdomain));
			} else if ($lang == 'en_GB') {
				$read_more_text = apply_filters('post_grid_filter_grid_item_read_more', __('... Read more »', post_grid_textdomain));
			}
			
			$html_content.= wp_trim_words(strip_shortcodes(get_the_excerpt()), $char_limit,''). ' <a target="'.$link_target.'" class="read-more d-inline-block" href="' . get_permalink() . '">' . $read_more_text . '</a>';
			$html_content.='</div>';
			}
